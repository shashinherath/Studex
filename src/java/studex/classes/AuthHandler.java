...
public class AuthHandler {
    ...
    public static String authenticateUser(String username, String password, HttpSession session) {
        ...
        try (Connection conn = DBHelper.getConnection(); PreparedStatement stmt = conn.prepareStatement(query)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    String sessionToken = TokenUtils.generateToken(username);
                    session.setAttribute("username", username);
                    session.setAttribute("sessionToken", sessionToken);
                    saveTokenToDatabase(username, sessionToken);  // Save the token in the database
    ...
    private static void saveTokenToDatabase(String username, String sessionToken) {
        ...
        try (Connection conn = DBHelper.getConnection(); PreparedStatement stmt = conn.prepareStatement(updateQuery)) {
            ...
            System.err.println("Error updating token for user: " + username);
            e.printStackTrace();
        }
    }
}
